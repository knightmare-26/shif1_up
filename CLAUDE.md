...
**Parked:** `LIVE_TIMING_ENABLED` in `src/config/features.ts` is `false`, so `/live` and `/live-monitor` show a "coming soon" notice (and the home page says so). FastF1 only serves a session after it finishes ("No data for this session!" mid-session), so a true live board needs a different source — OpenF1's paid tier or a client for F1's own SignalR live-timing feed. The code below is intact behind the flag; flip it to bring the pages back.
