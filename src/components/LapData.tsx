...
const LapData: React.FC = () => {
  ...
  const loadLapData = async () => {
    ...
    try {
      console.log(`🚀 Loading lap data for ${selectedYear} ${selectedGP} ${selectedSession}...`);
      
      const results = await backendApi.getSessionLaps(
        selectedYear, 
        selectedGP, 
        selectedSession, 
      ...
      console.log(`✅ Loaded ${results.laps.length} laps for ${selectedYear} ${selectedGP} ${selectedSession}`);
