...
- [x] **Verified live 2026-09-21** — with `SUPABASE_ACCESS_TOKEN` set on the Render API, the project was paused from the Supabase dashboard while the API was awake: `/health` noticed within seconds (`connecting`), the guardian saw `project_status: INACTIVE`, requested the restore itself, followed `COMING_UP` → `RESTORING`, and the database was `ready` again ~4 minutes later with no manual step; data endpoints, predictions and the trained models were intact afterwards. (Before the token was added the API reported `can_restore: false` and sat at `unavailable`.)
